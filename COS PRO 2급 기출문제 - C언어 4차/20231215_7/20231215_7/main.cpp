...
int func_a(int scores1[], int scores2[], int scores_len) // 가장 큰 차이를 반환하는 함수
{
    int answer = 0;
    for (int i = 0; i < scores_len; i++)
        // 현재까지의 answer보다 큰 차이가 있으면 answer를 갱신
        if (answer < scores2[i] - scores1[i])
            answer = scores2[i] - scores1[i];
    return answer;
}

int func_b(int scores1[], int scores2[], int scores_len) // 가장 작은 차이를 반환하는 함수
{
    int answer = 0;
    for (int i = 0; i < scores_len; i++)
